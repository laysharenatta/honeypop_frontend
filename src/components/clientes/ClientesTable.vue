<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            >
              ...
              <td class="px-6 py-1 text-right">
                <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    @click="handleInteractionHistory(cliente.id)"
      ...
      <HistorialInteraccionesModal
        :isOpen="isHistorialOpen"
        :clienteId="clienteIdHistorial"
        @close="isHistorialOpen = false"
      />
    </div>
  </div>
</template>
...
import HistorialInteraccionesModal from '../interacciones/HistorialInteraccionesModal.vue'
...
const clienteIdHistorial = ref(null)
...
const handleSave = async (data) => {
  ...
  try {
    ...
    if (mode.value === 'create') {
      ...
      await ClientesService.update(
        currentCliente.value.id,
        data
      )
    }
...
const handleInteractionHistory = (id) => {

  clienteIdHistorial.value = id
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await ClientesService.deleteCliente(
      idToDelete.value
    )
